...
public class RestComparisonsController {
    ...
    @ApiOperation(value = "get similarity between two vectors", nickname = "postCompare", response=Double.class)
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "Success", response = Double.class),
    })
    @RequestMapping(method = RequestMethod.POST, produces = "application/json")
    public Double index(@RequestBody ComparisonRequest request)  {
        try {
