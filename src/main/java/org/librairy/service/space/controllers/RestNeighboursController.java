...
public class RestNeighboursController {
    ...
    public NeighbourList similar(@RequestBody SimilarRequest request)  {
        try {
            return new NeighbourList(service.similar(request.getShape(), request.getNumber(), request.getType()).stream().map(n-> new org.librairy.service.space.rest.model.Neighbour(n)).collect(Collectors.toList()));
